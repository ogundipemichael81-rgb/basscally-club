...
assert.match(cont,/isAdminEmail\(user\.email\)/); assert.match(cont,/auth\.signOut\(\)/); assert.match(cont,/\/admin\/login/); assert.match(cont,/safeNext/);
...
assert.match(middleware,/!next\.startsWith\("\/admin"\)/); assert.match(layout,/requireAdminPage/); assert.match(layout,/force-dynamic/); assert.match(layout,/revalidate=0/);
